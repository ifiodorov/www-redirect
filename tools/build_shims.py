...
Третье правило - про поиск (добавлено 17.09.2026 после письма Search Console
«Проиндексировано, несмотря на блокировку в robots.txt» на адрес www):
  3. каждый шим обязан нести <meta name="robots" content="noindex">.
Запрет в robots.txt от индексации НЕ защищает: он запрещает СКАЧИВАТЬ страницу,
и Google, зная адрес со стороны, кладёт его в индекс пустой строкой - без
содержимого и без возможности это исправить, потому что noindex он тоже не
скачает. Поэтому www от поисковиков больше не закрыт, а помечен: робот заходит,
читает noindex и выбрасывает адрес из индекса. Краулеру Telegram мета-тег robots
безразличен - в отличие от canonical и meta refresh, он никуда не уводит.
"""
import html
import os
import re
...
def shim(og, path):
    ...
    lines = [
        ...
        "",
        "<!-- Для поиска этой страницы нет: индексируется только апекс. Помечаем",
        "     тегом, а не запретом в robots.txt - под запретом робот не скачает",
        "     страницу и не увидит пометку, а адрес всё равно осядет в индексе",
        "     пустой строкой. Краулеру Telegram этот тег безразличен. -->",
